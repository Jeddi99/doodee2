...
import { HAIR_COLORS, applyHairColor, type HairColor } from "@/lib/hair-recolor";
...
interface HairColorPanelProps {
  scan: ScanPhoto;
  onReset: () => void;
  /** Phase 57 — pixel-perfect hair mask from MediaPipe ImageSegmenter. */
  hairMask?: Uint8Array | null;
  hairMaskBusy?: boolean;
}

export function HairColorPanel({
  scan,
  onReset,
  hairMask,
  hairMaskBusy = false,
  ...
  const renderCanvas = useCallback(() => {
    ...
    try {
      applyHairColor(ctx, scan.landmarks, selected, {
        intensity,
        ...(hairMask ? { hairMask } : {}),
      });
    } catch {
  ...
  }, [hairMask, intensity, scan.image, scan.landmarks, selected, showOriginal]);
  ...
  async function handleDownload() {
    ...
    try {
      ...
      await saveBlob(blob, `doodee-hair-${tag}.png`);
      setDownloadOk("ok");
    } catch {
      setDownloadOk("err");
    } finally {
    ...
    <div className="w-full min-w-0 space-y-5 overflow-x-clip">
      ...
      <div className="relative mx-auto aspect-[3/4] w-full max-w-md overflow-hidden rounded-2xl border border-white/60 bg-white/35 shadow-[0_18px_52px_-40px_rgba(36,31,26,0.38)] backdrop-blur-md">
        <canvas
          ...
          aria-label={
            !selected || showOriginal
              ? t.hairColor.canvasAriaOriginal
              : t.hairColor.canvasAriaApplied.replace(
                  "{name}",
        ...
        <AnimatePresence>
          {selected && (
            ...
            >
              {showOriginal ? t.hairColor.original : t.hairColor.compareHold}
            </m.button>
          )}
        </AnimatePresence>
        {selected && (
        ...
        {hairMaskBusy && selected ? (
          <div
            role="status"
            aria-live="polite"
            className="absolute inset-x-3 bottom-3 rounded-2xl border border-white/60 bg-white/60 px-3 py-2 text-[11px] font-medium text-[#067e96] shadow-[0_12px_28px_-22px_rgba(36,31,26,0.4)] backdrop-blur-md"
          >
      ...
      <div className="space-y-2">
        <p className="text-[11px] uppercase tracking-[0.2em] text-[#766e65] text-center">
          {t.hairColor.paletteLabel}
        </p>
        <div className="mx-auto grid w-full max-w-md grid-cols-5 gap-2 sm:max-w-2xl sm:grid-cols-10">
          {HAIR_COLORS.map((c) => {
      ...
      <div className="flex min-w-0 flex-wrap items-center justify-center gap-3 pt-1">
        ...
        >
          {t.hairColor.changePhoto}
        </button>
      </div>

      <p className="text-center text-[10px] text-[#7c746d] leading-relaxed max-w-sm mx-auto">
        {t.hairColor.privacyNote}
      </p>
    </div>
  );
}

function localizedName(c: HairColor, lang: "th" | "en"): string {
