...
export function HairColorPage() {
  ...
  const [hairMaskBusy, setHairMaskBusy] = useState(false);
  ...
  // Phase 186 — Just null the local ref. HairSegmenter is now a
  // module-level singleton — disposing would close the shared WASM
  // context that the next page would need to reuse.
  useEffect(() => {
    return () => {
  ...
  async function handlePhoto(image: HTMLImageElement) {
    ...
    try {
      ...
      void runHairSegmentation(image, token);
    } catch {
      if (!mountedRef.current || uploadTokenRef.current !== token) {
        if (image.src.startsWith("blob:")) try { URL.revokeObjectURL(image.src); } catch {}
        return;
  ...
  async function runHairSegmentation(image: HTMLImageElement, token: number) {
    setHairMaskBusy(true);
    try {
      if (!segmenterRef.current) {
        segmenterRef.current = await HairSegmenter.load();
      }
      if (!mountedRef.current || uploadTokenRef.current !== token) return;
      const srcW = image.naturalWidth || image.width;
      const srcH = image.naturalHeight || image.height;
      if (srcW === 0 || srcH === 0) return;
      const mask = segmenterRef.current.segmentHairMask(image, srcW, srcH);
      if (mask && mountedRef.current && uploadTokenRef.current === token) {
        setHairMask(mask);
      }
    } catch {
    ...
    <div className="mx-auto w-full max-w-3xl min-w-0 space-y-8 overflow-x-clip">
      ...
      >
        {phase.kind === "idle" && (
          <PhotoUpload
            onPhoto={handlePhoto}
            promptText={t.hairColor.uploadPrompt}
          />
        )}
