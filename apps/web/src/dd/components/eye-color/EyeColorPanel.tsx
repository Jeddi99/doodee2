...
interface EyeColorPanelProps {
  scan: ScanPhoto;
  onReset: () => void;
}

export function EyeColorPanel({ scan, onReset }: EyeColorPanelProps) {
  const { t, lang } = useT();
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const [selected, setSelected] = useState<EyeColor | null>(null);
  ...
  const renderCanvas = useCallback(() => {
    ...
    try {
      applyEyeColor(ctx, scan.landmarks, selected, { intensity });
    } catch {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      await saveBlob(blob, `doodee-eye-${tag}.png`);
      setDownloadOk("ok");
    } catch {
      setDownloadOk("err");
    } finally {
    ...
    <div className="w-full min-w-0 space-y-5 overflow-x-clip">
      ...
      <div className="relative mx-auto aspect-[3/4] w-full max-w-md overflow-hidden rounded-2xl border border-white/60 bg-white/35 shadow-[0_18px_52px_-40px_rgba(36,31,26,0.38)] backdrop-blur-md">
        <canvas
          ...
          aria-label={
            !selected || showOriginal
              ? t.eyeColor.canvasAriaOriginal
              : t.eyeColor.canvasAriaApplied.replace(
                  "{name}",
        ...
        <AnimatePresence>
          {selected && (
            ...
            >
              {showOriginal ? t.eyeColor.original : t.eyeColor.compareHold}
            </m.button>
          )}
        </AnimatePresence>
        {selected && (
      ...
      <div className="space-y-2">
        <p className="text-[11px] uppercase tracking-[0.2em] text-[#766e65] text-center">
          {t.eyeColor.paletteLabel}
        </p>
        <div className="mx-auto grid w-full max-w-md gap-2 [grid-template-columns:repeat(5,_minmax(0,_1fr))] sm:max-w-2xl sm:[grid-template-columns:repeat(15,_minmax(0,_1fr))]">
          {EYE_COLORS.map((c) => {
      ...
      <div className="flex min-w-0 flex-wrap items-center justify-center gap-3 pt-1">
        ...
        >
          {t.eyeColor.changePhoto}
        </button>
      </div>

      <p className="text-center text-[10px] text-[#7c746d] leading-relaxed max-w-sm mx-auto">
        {t.eyeColor.privacyNote}
      </p>
    </div>
  );
}

function localizedName(c: EyeColor, lang: "th" | "en"): string {
